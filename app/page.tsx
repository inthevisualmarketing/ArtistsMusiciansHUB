...
export default function ComingSoon() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus("success"); setMessage("SIGNAL LOCKED — WE'LL FIND YOU SATURDAY"); setEmail("");
      } else {
        setStatus("error"); setMessage(data.error || "TRANSMISSION FAILED — TRY AGAIN");
      }
    } catch {
      setStatus("error"); setMessage("TRANSMISSION FAILED — TRY AGAIN");
    }
  };
  ...
  return (
    <main style={s.root}>
      ...
      <div style={s.horizon} />
      {/* Corners */}
      <div style={{ ...s.corner, ...s.tl }} /><div style={{ ...s.corner, ...s.tr }} />
      <div style={{ ...s.corner, ...s.bl }} /><div style={{ ...s.corner, ...s.br }} />
      {/* HUD top */}
      <div style={s.hudTop}>
      ...
      <div style={s.stage}>
        ...
        {launched ? (
          ...
          <div style={s.countdownWrap}>
            ...
            <div style={s.countdown}>
              {[{ val: time.d, label: "DAYS" }, { val: time.h, label: "HRS" }, { val: time.m, label: "MIN" }, { val: time.s, label: "SEC" }].map((u, i) => (
        ...
        <div style={s.emailSection}>
          ...
          <div style={s.emailRow}>
            ...
            >
              {status === "loading" ? "..." : status === "success" ? "✓ LOCKED" : "AMPLIFY"}
            </button>
          </div>
          {message && (
            <p style={{ ...s.statusMsg, color: status === "success" ? "#00ff41" : "#ff2a6d" }}>
              {message}
            </p>
          )}
        </div>
      ...
      <style>{`
        ...
        @keyframes symPulse { 0%,100%{opacity:.45} 50%{opacity:1} }
        .sym-1{animation:symPulse 1.8s 0s ease-in-out infinite}
        .sym-2{animation:symPulse 1.8s .3s ease-in-out infinite}
        .sym-3{animation:symPulse 1.8s .6s ease-in-out infinite}
        .sym-4{animation:symPulse 1.8s .9s ease-in-out infinite}
...
const s: Record<string, React.CSSProperties> = {
  ...
  stage: { position: "relative", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", gap: 24, textAlign: "center", padding: "0 24px" },
  symbols: { display: "flex", gap: 28, fontSize: 32, alignItems: "center" },
  sym: { fontSize: 32 },
  brand: { fontFamily: "'VT323',monospace", fontSize: "clamp(28px,7vw,64px)", letterSpacing: "0.25em", color: "#00ff41", textShadow: "0 0 10px #00ff41,0 0 30px rgba(0,255,65,0.5)", margin: 0, fontWeight: 400 },
  ...
  digitBox: { border: "1px solid rgba(0,255,65,0.2)", padding: "4px 12px", background: "rgba(0,255,65,0.03)", clipPath: "polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%)" },
  digit: { fontFamily: "'VT323',monospace", fontSize: "clamp(48px,9vw,80px)", lineHeight: 1, color: "#00ff41", textShadow: "0 0 8px #00ff41,0 0 24px rgba(0,255,65,0.4)", display: "block", minWidth: "2ch", textAlign: "center" },
  colon: { fontFamily: "'VT323',monospace", fontSize: "clamp(48px,9vw,80px)", color: "#00c832", lineHeight: 1, paddingBottom: 20, animation: "blink 1s step-end infinite", position: "absolute", right: "-16px", top: 0 },
  ...
  emailSection: { display: "flex", flexDirection: "column", alignItems: "center", gap: 10, width: "min(480px,90vw)" },
  ...
  tagline: { fontSize: 11, letterSpacing: "0.28em", color: "#1a4a26", margin: 0 },
};
